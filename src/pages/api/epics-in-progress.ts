...
export default async function EpicsInProgress(
  req: NextApiRequest,
  res: NextApiResponse,
) {
  ...
  try {
    const response = await axios.get(
      `${req.cookies.jiraHostUrl}/rest/api/3/search`,
      {
        ...
        params: {
          jql: `project in (${req.cookies.projects}) AND issuetype = Epic AND status != Done AND status != "Needs Estimate" ORDER BY updated DESC`,
